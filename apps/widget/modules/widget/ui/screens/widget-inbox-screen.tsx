...
const WidgetInboxScreen = () => {
  ...
  const conversations = usePaginatedQuery(api.public.conversations.getMany, contactSessionId ? {
    contactSessionId
  } : "skip", {
    initialNumItems: 10
  });
          ...
          >
            <div className="flex flex-col w-full gap-4 overflow-hidden text-start">
              <div className="flex w-full items-center justify-between gap-2">
                ...
                <p className="text-muted-foreground text-xs">{formatDistanceToNow(new Date(conversation.creationTime))}</p>
              </div>
              <div className="flex w-full items-center justify-between gap-2">
